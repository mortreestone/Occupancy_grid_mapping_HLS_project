...
def main(argv):
    ...
    #percent=(int(argv[3])%10)*10
    #length=int(int(argv[3])/1000)
    #integer=int(int(argv[3])/10)-(length*100)
    p= int(argv[3])
    a=float(int(p%10)/10)
    resolution=float(int((p-10*a)%10000)/800)


    report['design num'] = int(argv[3])
    ...
    if path.exists('reports.csv'):
        print("write in mama")
        with open('reports.csv', 'a') as output:
            writer = csv.DictWriter(output, fieldnames=fieldnames)
            writer.writerow(report)     
    else:
        with open('reports.csv','w') as output:
            print("write in baby")
            writer = csv.DictWriter(output, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerow(report)
